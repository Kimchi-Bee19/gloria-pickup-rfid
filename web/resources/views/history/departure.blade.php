    ...
    <x-slot name="content">
        ...
        <div class="flex-initial w-full justify-center items-center">
            <div class="mt-5 duration-500 text-center">
                ...
                <!-- Title -->
                <h1 class="text-3xl text-dark-blue font-bold mb-5">Daftar Departure Log</h1>

                <!-- Search & Add Vehicle Tag Button -->
                <div class="flex justify-between items-center mb-7">
                    <!-- Search -->
                    <div>
                        <form class="max-w-md" method="get" action="/departure-log/search">
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <!-- @php
                                $no = ($departures->currentPage() - 1) * $departures->perPage() + 1;
                            @endphp -->
                            ...
                            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td class="px-6 py-3">
                                    {{ $departure->student_identity->tag_id }}
                                </td>
                                <td class="px-6 py-3">
                                    {{ $departure->student->full_name }}
                                </td>
                                <td class="px-6 py-3">
                                    {{ $departure->student->class }}
                                </td>
                                <td class="px-6 py-3">
                                    {{ $departure->student->internal_id }}
                                </td>
                                <td class="px-6 py-3">
                                    {{ date(' H:i:s d-M-Y ', strtotime($departure->departure_time)) }}
                                </td>
                            </tr>
                ...
                <div class="flex items-center justify-between mb-5 mt-3">
                    <!-- Showing Text -->
                    <span class="text-sm">
                        Showing <span class="font-bold">{{ $departures->firstItem() }}</span> to <span class="font-bold">{{ $departures->lastItem() }}</span> of <span class="font-bold">{{ $departures->total() }}</span> entries
                    </span>
                ...
                <div class="flex justify-center mt-7">
                    <nav aria-label="Page navigation example">
                        <ul class="flex items-center -space-x-px h-10 text-base">
                            <!-- Tombol Previous -->
                            <li>
                                <a href="{{ $departures->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ $departures->onFirstPage() ? 'cursor-not-allowed opacity-50' : '' }}" {{ $departures->onFirstPage() ? 'aria-disabled=true' : '' }}>
                            ...
                            <!-- Links Halaman -->
                            @foreach(range(1, $departures->lastPage()) as $i)
                                <li>
                                    <a href="{{ $departures->url($i) }}" class="flex items-center justify-center px-4 h-10 leading-tight {{ $departures->currentPage() == $i ? 'text-blue-600 border border-blue-300 bg-blue-50' : 'text-gray-500 bg-white border border-gray-300' }} hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                        {{ $i }}
                                    </a>
                                </li>
                            @endforeach
                            ...
                            <!-- Tombol Next -->
                            <li>
                                <a href="{{ $departures->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ !$departures->hasMorePages() ? 'cursor-not-allowed opacity-50' : '' }}" {{ !$departures->hasMorePages() ? 'aria-disabled=true' : '' }}>
