    ...
    <x-slot name="content">
        ...
        <div class="flex-initial w-full justify-center items-center">
            <div class="mt-5 duration-500 text-center">
                ...
                <!-- Title -->
                <h1 class="text-3xl text-dark-blue font-bold mb-5">Daftar Arrival Tracking</h1>

                <!-- Search & Add Vehicle Tag Button -->
                <div class="flex justify-between items-center mb-7">
                    <!-- Search -->
                    <div>
                        <form class="max-w-md" method="get" action="/arrival-departure-tracking/search">
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <!-- @php
                                $no = ($trackings->currentPage() - 1) * $trackings->perPage() + 1;
                            @endphp -->
                            ...
                            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td class="px-6 py-3">
                                    {{ date(' H:i:s d-M-Y ', strtotime($tracking->vehicle_arrival_log->arrival_time)) }}
                                </td>
                                <td class="px-6 py-3">
                                    @if ($tracking->student_departure_logs->isEmpty())
                                        No departure record for the student yet.
                                    @else
                                        @foreach($tracking->student_departure_logs as $student_departure_log)
                                            <div>{{ date(' H:i:s d-M-Y ', strtotime($student_departure_log->departure_time)) }}</div>
                                        @endforeach
                                    @endif
                                </td>
                                <td class="px-6 py-3 text-left">
                                    <ul>
                                        <li>Tag ID: {{ $tracking->vehicle_arrival_log->vehicle_identity->tag_id ?: "" }}</li>
                                        <li>Type: {{ $tracking->vehicle_arrival_log->vehicle->type }}</li>
                                        <li>Model: {{ $tracking->vehicle_arrival_log->vehicle->model }}</li>
                                        <li>Color: {{ $tracking->vehicle_arrival_log->vehicle->color }}</li>
                                        <li>License Plate: {{ $tracking->vehicle_arrival_log->vehicle->license_plate }}</li>
                                    </ul>
                                </td>
                                <td class="px-6 py-3 text-left">
                                    <ul>
                                        ...
                                        @else
                                           @foreach($tracking->student_departure_logs as $student_departure_log)
                                                <li>Tag ID: {{ $student_departure_log->student_identity->tag_id }}</li>
                                                <li>Nama: {{ $student_departure_log->student->full_name }}</li>
                                                <li>Kelas: {{ $student_departure_log->student->class }}</li>
                                            @endforeach
                                        @endif
                                    </ul>
                                </td>
                ...
                <div class="flex items-center justify-between mb-5 mt-3">
                    <!-- Showing Text -->
                    <span class="text-sm">
                        Showing <span class="font-bold">{{ $trackings->firstItem() }}</span> to <span class="font-bold">{{ $trackings->lastItem() }}</span> of <span class="font-bold">{{ $trackings->total() }}</span> entries
                    </span>
                ...
                <div class="flex justify-center mt-7">
                    <nav aria-label="Page navigation example">
                        <ul class="flex items-center -space-x-px h-10 text-base">
                            ...
                            <!-- Links Halaman -->
                            @foreach(range(1, $trackings->lastPage()) as $i)
                                <li>
                                    <a href="{{ $trackings->url($i) }}" class="flex items-center justify-center px-4 h-10 leading-tight {{ $trackings->currentPage() == $i ? 'text-blue-600 border border-blue-300 bg-blue-50' : 'text-gray-500 bg-white border border-gray-300' }} hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                        {{ $i }}
                                    </a>
                                </li>
                            @endforeach
                            ...
                            <!-- Tombol Next -->
                            <li>
                                <a href="{{ $trackings->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ !$trackings->hasMorePages() ? 'cursor-not-allowed opacity-50' : '' }}" {{ !$trackings->hasMorePages() ? 'aria-disabled=true' : '' }}>
