    ...
    <x-slot name="content">
        ...
        <div class="flex-initial w-full justify-center items-center">
            <div class="mt-5 duration-500 text-center">
                ...
                <!-- Title -->
                <h1 class="text-3xl text-dark-blue font-bold mb-5">Daftar Tag Siswa</h1>

                <!-- Search & Add Student Tag Button -->
                <div class="flex justify-between items-center mb-7">
                    <!-- Search -->
                    <div>
                        <form class="max-w-md" method="get" action="/tag_siswa/search">
                    ...
                    <div class = "flex gap-3">
                        <!-- Add Student Tag Button -->
                    <div class="flex">
                        <a href = '{{ route('student-identity.create') }}'
                            ...
                            <div>Tambah Tag Siswa</div>
                        </a>
                    </div>

                    <!-- Import Student Tag -->
                    <div class="flex">
                        <a data-modal-target="import-student-identity-popup" data-modal-toggle="import-student-identity-popup"
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            <!-- @php
                                $no = ($student_identities->currentPage() - 1) * $student_identities->perPage() + 1;
                            @endphp -->
                            @forelse($student_identities as $student_identity)
                            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $student_identity['id'] }}
                                </th>
                                <td class="px-6 py-3">
                                    {{ $student_identity['type'] }}
                                </td>
                                <td class="px-6 py-3">
                                    {{ $student_identity['tag_id'] }}
                                </td>
                                <td class="px-6 py-3">
                                    {{ $student_identity['notes'] }}
                                </td>
                                <td class="px-6 py-3">
                                    <a href="{{ route('student-identity.edit', $student_identity->id) }}" class="bg-green-500 text-white font-medium py-2 px-4 rounded-full hover:bg-green-600 transition duration-300 inline-flex items-center">
                                    ...
                                    <button data-modal-target="popup-modal-{{ $student_identity->id }}" data-modal-toggle="popup-modal-{{ $student_identity->id }}" type="button" class="bg-red-500 text-white font-medium py-2 px-4 rounded-full hover:bg-red-600 transition duration-300 ml-2 inline-flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5 inline-block align-middle mr-1">
                            ...
                            @include('student-identities.delete')

                            @empty
                                <tr>
                                    <td colspan="9" class="px-6 py-4 text-center text-gray-500">
                                        No student tags are found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                {{-- <div class="flex items-center justify-between mb-5 mt-3">
                    <!-- Showing Text -->
                    <span class="text-sm">
                        Showing <span class="font-bold">{{ $student_identities->firstItem() }}</span> to <span class="font-bold">{{ $student_identities->lastItem() }}</span> of <span class="font-bold">{{ $student_identities->total() }}</span> entries
                    </span>

                    <!-- Dropdown -->
                    <div class="flex items-center">
                        <form method="get" action="{{ url('/tag_siswa') }}" id="perPageForm">
                ...
                {{-- <div class="flex justify-center mt-7">
                    <nav aria-label="Page navigation example">
                        <ul class="flex items-center -space-x-px h-10 text-base">
                            <!-- Tombol Previous -->
                            <li>
                                <a href="{{ $student_identities->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ $student_identities->onFirstPage() ? 'cursor-not-allowed opacity-50' : '' }}" {{ $student_identities->onFirstPage() ? 'aria-disabled=true' : '' }}>
                            ...
                            <!-- Links Halaman -->
                            @foreach(range(1, $student_identities->lastPage()) as $i)
                                <li>
                                    <a href="{{ $student_identities->url($i) }}" class="flex items-center justify-center px-4 h-10 leading-tight {{ $student_identities->currentPage() == $i ? 'text-blue-600 border border-blue-300 bg-blue-50' : 'text-gray-500 bg-white border border-gray-300' }} hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                        {{ $i }}
                                    </a>
                                </li>
                            @endforeach
                            ...
                            <!-- Tombol Next -->
                            <li>
                                <a href="{{ $student_identities->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ !$student_identities->hasMorePages() ? 'cursor-not-allowed opacity-50' : '' }}" {{ !$student_identities->hasMorePages() ? 'aria-disabled=true' : '' }}>
                ...
                <div class = "mt-10">
                    {{$student_identities->links()}}
                </div>
            </div>
        </div>
        @include('student-identities.import')
    </x-slot>
