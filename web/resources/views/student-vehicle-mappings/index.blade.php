    ...
    <x-slot name="content">
        ...
        <div class="flex-initial w-full justify-center items-center">
            <div class="mt-5 duration-500 text-center">
                ...
                <!-- Title -->
                <h1 class="text-3xl text-dark-blue font-bold mb-5">Daftar Asosiasi Siswa dan Kendaraan</h1>

                <!-- Search & Add Vehicle Button -->
                <div class="flex justify-between items-center mb-7">
                    <!-- Search -->
                    <div class="flex-none">
                        <form class="max-w-md" method="get" action="/student-vehicle-mapping/search">
                    ...
                    <div class="flex space-x-3">
                        <!-- Add Vehicle Button -->
                        <a href="{{ route('student-vehicle-mapping.create') }}" class="flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                ...
                <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                    <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                            ...
                            <tr>
                                ...
                                <th scope="col" class="px-6 py-3">
                                    <div class="flex items-center justify-center">
                                        Kelas
                                        <a href="#">
                                            <svg class="w-3 h-3 ms-1.5" aria-hidden="true" onclick="sortTable(7)" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z"/>
                                            </svg>
                                        </a>
                                    </div>
                        ...
                        <tbody>
                            <!-- @php
                                $no = ($student_vehicle_mappings->currentPage() - 1) * $student_vehicle_mappings->perPage() + 1;
                            @endphp -->
                            @forelse($student_vehicle_mappings as $mapping)
                            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $mapping['id'] }}
                                </th>
                                <td class="px-6 py-3">
                                    {{ $mapping->vehicle->license_plate }}
                                </td>
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3">
                                    <a href="{{ route('student-vehicle-mapping.edit', $mapping->id) }}" class="bg-green-500 text-white font-medium py-2 px-4 rounded-full hover:bg-green-600 transition duration-300 inline-flex items-center">
                                    ...
                                    <button data-modal-target="popup-modal-{{ $mapping->id }}" data-modal-toggle="popup-modal-{{ $mapping->id }}" type="button" class="bg-red-500 text-white font-medium py-2 px-4 rounded-full hover:bg-red-600 transition duration-300 ml-2 inline-flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5 inline-block align-middle mr-1">
                            ...
                            @include('student-vehicle-mappings.delete')

                            @empty
                                <tr>
                                    <td colspan="9" class="px-6 py-4 text-center text-gray-500">
                                        No student and vehicle association are found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="flex items-center justify-between mb-5 mt-3">
                    <!-- Showing Text -->
                    <span class="text-sm"> 
                        Showing <span class="font-bold">{{ $student_vehicle_mappings->firstItem() }}</span> to <span class="font-bold">{{ $student_vehicle_mappings->lastItem() }}</span> of <span class="font-bold">{{ $student_vehicle_mappings->total() }}</span> entries
                    </span>

                    <!-- Dropdown -->
                    <div class="flex items-center">
                        <form method="get" action="{{ url('/student-vehicle-mapping') }}" id="perPageForm">
                ...
                <div class="flex justify-center mt-7">
                    <nav aria-label="Page navigation example">
                        <ul class="flex items-center -space-x-px h-10 text-base">
                            <!-- Tombol Previous -->
                            <li>
                                <a href="{{ $student_vehicle_mappings->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ $student_vehicle_mappings->onFirstPage() ? 'cursor-not-allowed opacity-50' : '' }}" {{ $student_vehicle_mappings->onFirstPage() ? 'aria-disabled=true' : '' }}>
                            ...
                            <!-- Links Halaman -->
                            @foreach(range(1, $student_vehicle_mappings->lastPage()) as $i)
                                <li>
                                    <a href="{{ $student_vehicle_mappings->url($i) }}" class="flex items-center justify-center px-4 h-10 leading-tight {{ $student_vehicle_mappings->currentPage() == $i ? 'text-blue-600 border border-blue-300 bg-blue-50' : 'text-gray-500 bg-white border border-gray-300' }} hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                        {{ $i }}
                                    </a>
                                </li>
                            @endforeach
                            ...
                            <!-- Tombol Next -->
                            <li>
                                <a href="{{ $student_vehicle_mappings->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white {{ !$student_vehicle_mappings->hasMorePages() ? 'cursor-not-allowed opacity-50' : '' }}" {{ !$student_vehicle_mappings->hasMorePages() ? 'aria-disabled=true' : '' }}>
