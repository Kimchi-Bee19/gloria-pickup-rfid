...
bool ESP32MQTTClient::publish(const String &topic, const char *payload, int len, int qos, bool retain)
{
    // Do not try to publish if MQTT is not connected.
    if (!isConnected()) //! isConnected())
    {
    ...
    bool success = false;
    if (esp_mqtt_client_publish(_mqtt_client, topic.c_str(), payload, len, qos, retain) != -1)
    {
        success = true;
    }
